...
! 教学式注释: Buffer #27 主程序初始化与系统调度
! 目的: 初始化控制器参数/轴配置/互锁/通讯, 并启动各子Buffer。
! 入口条件: AUTOEXEC 自动执行, 控制器上电或程序重启后进入。
! 输出效果: 设定轴参数、加载持久化配置、启动通讯/互锁/机器人/QCM等Buffer。
...
! ---- 自动执行入口 ----
! 输入条件: 控制器启动或程序加载。
! 输出效果: 清理状态, 停止其他Buffer, 进入初始化。
AUTOEXEC:
! 清除运动/缓冲残留状态, 防止启动时有未预期运动
KILLALL
!STOP ALL Buffers Except for a VAT control buffer
INT j
j=0
! 扫描并停止其他Buffer, 保留自身与特定例外Buffer
loop 32
	if (j<7) 
		if (iType(j)<>1040)
			STOP j
...
!====ZW on 2/4/2016====
! ---- 全局/共享变量 ----
! 这些变量被多个Buffer共享, 多数映射到TAG/状态字(见Buffer #A)或上位机交互。
...
! TAG: AxisInfo/AxisLog 等用于上位机读写的状态/日志数组(见Buffer #A映射)
...
! MFLAGS: 轴标志字(如#HOME/制动/默认连接)，表示回零完成与轴配置状态
...
! MST: 轴运行状态字(如#ENABLED、.4=运动状态)，用于判断是否允许运动/完成移动
